...
export const Navbar: React.FC<NavbarProps> = ({ userId, userName, onLogout }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribeAuth = auth.onAuthStateChanged((user: User | null) => {
            if (user && user.uid === userId) {
                ...
                unsubscribeTimer = onSnapshot(timerDocRef, (docSnap) => {
                    if (docSnap.exists()) {
                        const data = docSnap.data() as UserTimer;
                        const serverTime = data.remainingTime;

                        // Only update from server if:
                        // 1. Initial load (localTimeRef.current === 0 and not expired state)
                        // 2. Server time is SIGNIFICANTLY greater than local time (admin added time)
                        // We use a threshold (e.g., 5 seconds) to prevent jitter/lag updates
                        const isSignificantlyGreater = serverTime > (localTimeRef.current + 5);

                        // If local is 0 (expired), only update if server has meaningful time added (e.g. > 30s)
                        // This prevents the "27s remaining" stale data from resetting the expired modal
                        const isValidReset = localTimeRef.current === 0 ? serverTime > 30 : isSignificantlyGreater;

                        if (localTimeRef.current === 0 && !remainingTime) {
                            // Initial load
                            ...
                            // Reset expired state if admin added time
                            if (serverTime > 0) {
                                setIsExpired(false);
    ...
    useEffect(() => {
        ...
        // Only start listener when we are sure the user is authenticated and matches the prop
        // This prevents "Permission Denied" errors if the component mounts before Auth SDK is ready
        const unsubscribeAuth = auth.onAuthStateChanged((user: User | null) => {
            if (user && user.uid === userId) {
                const q = query(
                ...
                // If logged out or user mismatch, clear notifications
                setNotifications([]);
                setUnreadCount(0);
    ...
    useEffect(() => {
        ...
        timerRef.current = setInterval(() => {
            setRemainingTime((prev) => {
                ...
                if (prev <= 0) {
                    // If just hit 0 and not yet marked expired
                    if (prev === 0 && !isExpired) {
                        setIsExpired(true);
                        setShowExpiredModal(true);
                        localTimeRef.current = 0;
                        // Force immediate sync to 0
                        syncToFirestore(0);
                    }
                    return 0;
                }
                ...
                // Check if we need to sync to Firestore
                const now = Date.now();
    ...
    const syncToFirestore = async (currentSeconds: number) => {
        if (!userId) return;
        try {
            const timerDocRef = doc(db, 'userTimers', userId);
    ...
    const handleContactAdmin = () => {
        ...
        window.location.href = 'mailto:admin@accounting.uz?subject=Vaqt%20Tugadi%20-%20To\'lov%20Qilish';
    };
    ...
    return (
        <>
            ...
            <nav className="fixed top-0 w-full z-50 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 text-white px-6 py-3 flex justify-between items-center shadow-lg">
                ...
                {/* Center - Search or Nav Links (Placeholder) */}
                <div className="hidden md:flex items-center gap-6 text-sm text-slate-400">
                    <a href="/dashboard" className="hover:text-white transition-colors">Dashboard</a>
                    <a href="/leaderboard" className="hover:text-white transition-colors">Reyting</a>
                </div>
                                                ...
                                                {notifications.map((notif) => (
                                                    ...
                                                    >
                                                        <div className="flex gap-3">
                                                            <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${!notif.read ? 'bg-purple-500' : 'bg-slate-600'}`}></div>
                                                            <div>
                                                                <p className={`text-sm font-medium mb-1 ${!notif.read ? 'text-white' : 'text-slate-400'}`}>
                                                                    {notif.title}
                                                                </p>
